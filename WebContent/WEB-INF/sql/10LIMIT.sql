...
SELECT * FROM Customers LIMIT 3; -- 조회결과 중 첫번째 3개 레코드만 출력
...
-- Product 테이블 조회해서 가장 가격이 비싼 상품 3개 조회
SELECT * FROM Products ORDER BY Price DESC LIMIT 3;

-- LIMIT [부터], [몇개]
SELECT * FROM Customers LIMIT 0, 3;

-- Product 테이블 조회해서 두번째로 가격이 비싼 상품부터 4번째로 가격이 비싼 상품 조회
...
-- 페이징 (한 페이지에 10개)
SELECT * FROM Customers ORDER BY CustomerID LIMIT 0, 10; -- 1PAGE
SELECT * FROM Customers ORDER BY CustomerID LIMIT 10, 10; -- 2PAGE
SELECT * FROM Customers ORDER BY CustomerID LIMIT 20, 10; -- 3PAGE
